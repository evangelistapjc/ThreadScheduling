...
void edf_schedule(void) {
	...
	if (readyQue.size() > 0) {
		std::vector<int>::iterator priority = readyQue.begin();
		for(std::vector<int>::iterator iterate = readyQue.begin() + 1; iterate != readyQue.end(); iterate++) {
			if(tcb[*iterate].deadline < tcb[*priority].deadline)
				priority = iterate;
		}

		int thread = *priority;
		readyQue.erase(priority);
		pthread_cond_signal(&cond[thread]);
	}
	pthread_mutex_unlock(&mutex);
}
...
void rm_schedule(void) {
	...
	if(!readyQue.empty()) {
    	std::vector<int>::iterator priority = readyQue.begin();
    	for(std::vector<int>::iterator iterate = readyQue.begin() + 1; iterate != readyQue.end(); iterate++) {
      		if(tcb[*iterate].task_time < tcb[*priority].task_time)
        		priority = iterate;
    	}
